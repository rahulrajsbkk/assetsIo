    ...
    if (transCoin !== 'USD') {
      Axios.post(
        'https://gxtokenoptions.azurewebsites.net/api/Trade/ConvertCryptoUSD',
        {
          ...
          affiliate_id: '',
          coin_purchased: 'USD',
          purchased_from: transCoin,
          from_amount: depositAsset / coinObject.price,
        }
      ).then((res) => {
    ...
    <>
      {loading ? (
              ...
              {message.status ? (
                <>
                  ...
                  <h4 className="text-white text-center mx-4 mb-auto">
                    ${depositAsset} Has Been Deposited Into Your Options Account
                  </h4>
                </>
              ) : (
        ...
        <div className="set-amount">
          ...
          {isCoinSelect ? (
            <div className="deposit-footer">Change "From" Vault</div>
          ) : (
            <div
